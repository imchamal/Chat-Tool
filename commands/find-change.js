...
function runFind(keyword) {
    ...
    const panel = createPanel('ct-find-panel', resultTitleHtml(keyword, count), () => clearHighlights());
...
function showChangeResultPanel(find, replaceValue) {
    const panel = createPanel('ct-change-panel', resultTitleHtml(find, getMarkCount()), () => clearHighlights());
    const body = getPanelBody(panel);
    updatePositionLabel(panel);
...
export function registerFindChangeCommands() {
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        name: 'change',
        helpString: '채팅에서 찾아 바꿉니다. 검색 후 검토/바꾸기를 선택합니다. 사용법: /change 원본텍스트/바꿀텍스트, 또는 인자 없이 /change 만 입력하면 입력 패널이 뜹니다.',
        unnamedArgumentList: [
